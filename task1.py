...
def logger(old_function):
    ...
    start = start_now.isoformat()
    def new_function(*args, **kwargs):
        result = old_function(*args, **kwargs)
        logging.basicConfig(level=logging.INFO, filename='main.log', encoding='UTF-8', force=True)
        logging.info({
            'дата и время вызова функции': start,
            'имя функции': f'{old_function.__name__}',
            'аргументы': f'{args},{kwargs}',
